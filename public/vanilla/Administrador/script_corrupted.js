...
const Utils = {
  ...
  /**
   * Sistema mejorado de carga de imÃ¡genes con fallbacks
   */
  ...
  /**
   * Obtener informaciÃ³n botÃ¡nica enriquecida por especie
   */
  getEnrichedSpeciesInfo(species) {
    ...
    // Base de datos de informaciÃ³n botÃ¡nica
    const speciesDB = {
      'Madera Amarilla': {
        family: 'Fabaceae',
        distribution: 'Suroeste de EE.UU., noroeste de MÃ©xico',
        habitat: 'Bosques secos, zonas Ã¡ridas',
        characteristics: {
          forma: 'Ãrbol mediano, copa densa',
          hoja: 'Compuesta, bipinnada',
          tronco: 'Corteza lisa, gris claro',
          mantenimiento: 'Bajo'
        },
        observations: [
          'Resistente a sequÃ­as prolongadas',
      ...
      'Mezquite Dulce': {
        ...
        habitat: 'Zonas Ã¡ridas urbanas, suelos rocosos',
        characteristics: {
          forma: 'Ãrbol bajo, copa extendida',
          hoja: 'Bipinnada, verde claro',
          tronco: 'Corteza rugosa, cafÃ© oscuro',
          mantenimiento: 'Muy bajo'
        },
        observations: [
          'Extremadamente resistente a sequÃ­a',
          'RaÃ­ces profundas (hasta 50m)',
          'Produce vainas comestibles'
        ],
        biodiversity: [
          'Alimento para fauna desÃ©rtica',
      ...
      'Palo Verde': {
        ...
        characteristics: {
          forma: 'Ãrbol mediano, corteza verde',
          hoja: 'PequeÃ±as, caducas en sequÃ­a',
          tronco: 'Verde fotosintÃ©tico',
          mantenimiento: 'Muy bajo'
        },
        observations: [
          'FotosÃ­ntesis en corteza verde',
          'Pierde hojas en sequÃ­a extrema',
          'FloraciÃ³n amarilla espectacular'
        ],
        biodiversity: [
          'Ãrbol estatal de Arizona',
          'NÃ©ctar para abejas nativas',
          'Refugio para aves migratorias'
        ]
      }
    };
    
    // InformaciÃ³n por defecto si no se encuentra
    const defaultInfo = {
      family: 'No disponible',
      distribution: 'RegiÃ³n sonorense',
      habitat: 'Zonas Ã¡ridas urbanas',
      characteristics: {
        forma: 'Ãrbol urbano adaptado',
        hoja: 'Adaptada al clima Ã¡rido',
        tronco: 'Resistente',
...
const MapManager = {
  ...
  init() {
    ...
    try {
      ...
      console.error('âŒ Error inicializando mapa:', error);
      throw error;
    }
  },
  
  /**
   * Cargar Ã¡rboles en el mapa
   */
  async loadTrees() {
    try {
      ...
      if (!geojsonData?.features?.length) {
        throw new Error('No se encontraron datos de Ã¡rboles vÃ¡lidos');
      }
  ...
  bindTreePopup(feature, layer) {
    ...
    // Obtener informaciÃ³n botÃ¡nica enriquecida
    ...
    const popupContent = `
      <div class="tree-popup-botanical">
        ...
        <div class="taxonomic-info">
          ...
          <div class="tax-row">
            <span class="tax-label">DistribuciÃ³n:</span>
            <span class="tax-value">${info.distribution}</span>
          </div>
          <div class="tax-row">
            <span class="tax-label">HÃ¡bitat:</span>
            <span class="tax-value">${info.habitat}</span>
          </div>
        </div>
        
        <!-- Datos generales con iconografÃ­a -->
        ...
        <div class="uses-section">
          <div class="uses-grid">
            <div class="use-category">
              <div class="use-header">PAISAJÃSTICOS</div>
              <div class="use-icons">
  ...
  applyFilters() {
    ...
    // Actualizar estadÃ­sticas con datos filtrados
...
const UI = {
  ...
  /**
   * Configurar navegaciÃ³n principal
   */
  setupNavigation() {
      ...
      locateBtn.addEventListener('click', () => {
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(
            ...
            error => {
              console.warn('GeolocalizaciÃ³n no disponible:', error);
              this.showError('No se pudo obtener tu ubicaciÃ³n');
            }
          );
        }
      });
  ...
  /**
   * Configurar filtros de exploraciÃ³n
   */
  setupFilters() {
   ...
   */
  showOverview() {
    document.getElementById('panel-title').textContent = 'EstadÃ­sticas del Bosque Urbano';
  ...
  updateTreeContent(tree) {& props.photo_url.trim() !== '') {
    ...
    if (treeImage) {defecto
      ...
      // 1. URL especÃ­fica del Ã¡rboleImage, imageUrls[0], imageUrls.slice(1));
      if (props.photo_url && props.photo_url.trim() !== '') {
        imageUrls.push(props.photo_url);
      }
       Detalles del Ã¡rbol
      // 2. URL por nombre comÃºnmeter', props.diameter_cm ? `${props.diameter_cm} cm` : 'No disponible');
    ...
    *
    // Detalles del Ã¡rbols individuales del Ã¡rbol
  ...
  updateIndividualBenefits(benefits) {
    ...
    console.log('ðŸ” Container encontrado:', container);</div>
          ...
          <div class="tree-card-content">
            ...
            <div class="tree-card-stats">
              ...
              <div class="tree-card-stat">
                ...
                <span class="tree-card-stat-label">cm diÃ¡metro</span>
              </div>
            </div>
          </div>
        </div>
  ...
  performGlobalSearch(query) {
    ...
    if (!query.trim()) {
      resultsContainer.innerHTML = '<p class="no-results">Escribe para buscar Ã¡rboles...</p>';
      return;
    }
  ...
  /**
   * Renderizar grÃ¡ficos de beneficios
   */
  ...
  showError(message) {
    ...
    // TODO: Implementar notificaciÃ³n de error visual
  }
};
  ...
  async init() {
    try {
      ...
      if (typeof L === 'undefined') {
        throw new Error('Leaflet no estÃ¡ disponible');
      }
      
      if (typeof Chart === 'undefined') {
        console.warn('Chart.js no disponible - grÃ¡ficos deshabilitados');
      }
