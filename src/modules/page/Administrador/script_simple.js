// ðŸŒ³ HERMOSILLO TREE MAP - Script Principal
...
async function loadTrees() {
  try {
    console.log('ðŸŒ³ Cargando Ã¡rboles...');
    ...
    if (!geojsonData || !geojsonData.features || !Array.isArray(geojsonData.features)) {
      throw new Error('Estructura de datos GeoJSON invÃ¡lida');
    }
    
    console.log('âœ… Datos cargados:', geojsonData.features.length, 'Ã¡rboles');
    ...
    treeLayer = L.geoJSON(geojsonData, {
      ...
      onEachFeature: function(feature, layer) {
        ...
        layer.bindPopup(`
          ...
          <p><strong>DiÃ¡metro:</strong> ${props.diameter_cm} cm</p>
    ...
    treeLayer.addTo(map);
    console.log('âœ… Ãrboles aÃ±adidos al mapa');
...
function updateWelcomePanelStats(stats) {
  ...
  document.getElementById('avg-age').textContent = `${stats.avgAge} aÃ±os`;
  ...
  document.getElementById('most-common-details').textContent = 
    `${stats.speciesCount[stats.mostCommonSpecies]} Ã¡rboles encontrados`;
}
...
function updateTreeDetails(feature) {
  console.log('ðŸŒ³ Mostrando detalles del Ã¡rbol');
...
function setupSidebarToggle() {
  console.log('ðŸ“± Configurando toggle del sidebar...');
  ...
  if (sidebarToggle && sidebar) {
    sidebarToggle.addEventListener('click', function() {
      ...
      sidebarToggle.innerHTML = sidebar.classList.contains('active') ? 'âŒ Cerrar' : 'ðŸ“‹ Info';
      
      if (selectedTreeData) {
        showWelcomePanel();
      }
    });
    console.log('âœ… Toggle del sidebar configurado');
  }
}
...
function setupNavigation() {
  ...
  navLinks.forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      // AÃ±adir clase active al link clickeado
    ...
    loadingEl.innerHTML = `
      <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(45, 80, 22, 0.9); display: flex; align-items: center; justify-content: center; z-index: 9999; color: #FEFCF3; font-family: 'Inter', sans-serif;">
        <div style="text-align: center;">
          ...
          <p>Cargando Ã¡rboles de Hermosillo...</p>
        </div>
      </div>
    `;
    
    // AÃ±adir animaciÃ³n de spinner
...
function showErrorMessage(error) {
  const errorMessage = error.name === 'AbortError' 
    ? 'La carga de datos tomÃ³ demasiado tiempo. Verifica tu conexiÃ³n.'
    : `Error: ${error.message}`;
  ...
  errorEl.innerHTML = `
    <strong>âš ï¸ Error</strong><br>${errorMessage}<br><br>
...
document.addEventListener('DOMContentLoaded', function() {
  console.log('ðŸ“„ DOM listo, inicializando aplicaciÃ³n...');
  
  try {
    // Verificar dependencias
    console.log('ðŸ” Verificando dependencias:');
    console.log('  - Leaflet:', typeof L !== 'undefined' ? 'âœ…' : 'âŒ');
    console.log('  - Chart.js:', typeof Chart !== 'undefined' ? 'âœ…' : 'âŒ');
    
    if (typeof L === 'undefined') {
      throw new Error('Leaflet.js no estÃ¡ cargado');
    }
    ...
    // Cargar Ã¡rboles despuÃ©s de que el mapa estÃ© listo
    setTimeout(() => {
      loadTrees();
    }, 500);
    
    console.log('âœ… Hermosillo Tree Map inicializada');
    
  } catch (error) {
    console.error('âŒ Error inicializando la aplicaciÃ³n:', error);
    showErrorMessage(error);
  }
});
