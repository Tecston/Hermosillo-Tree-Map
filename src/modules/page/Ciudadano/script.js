...
const APP_CONFIG = {
  ...
  lstLegend: {
    ...
    minLabel: '25°',        // lo que quieras al lado izquierdo de la barra
    maxLabel: '38°',        // lo que quieras al lado derecho de la barra
...
const MapManager = {
  ...
  async loadColonias() {
    try {
      ...
      coloniasFC.features.forEach((f,i)=>{ f.properties ||= {}; if (f.properties._uid == null) f.properties._uid = i+1; });
      ...
      // ---------------------------
      // NIVEL 2: PolÃ­gonos (colonias) con opacidad fija
      // ---------------------------
      const FILL_COLOR       = '#c8e6c9';
      const FILL_COLOR_HOVER = '#88c758';
      ...
      if (!AppState.map.getLayer(AppState.coloniasFillLayer)) {
        AppState.map.addLayer({
          ...
          paint: {
            'fill-color': [
              'case',
              ['boolean', ['feature-state', 'hover'], false], FILL_COLOR_HOVER,
              FILL_COLOR
            ],
            'fill-opacity': [
              'case',
